...
function fish_prompt
  ...
  set -l git_icon_color (set_color magenta)

  set -l prompt_string (whoami)

  if test "$theme_ignore_ssh_awareness" != 'yes' -a -n "$SSH_CLIENT$SSH_TTY"
    set prompt_string "$fish "(whoami)"@"(hostname -s)
  end
  ...
  if git_is_repo
    ...
    echo -n -s " " $normal_color \[ $directory_color $cwd $normal_color \]
    echo -n -s "$git_icon_color  " $repository_color (git_branch_name) $normal_color " "
